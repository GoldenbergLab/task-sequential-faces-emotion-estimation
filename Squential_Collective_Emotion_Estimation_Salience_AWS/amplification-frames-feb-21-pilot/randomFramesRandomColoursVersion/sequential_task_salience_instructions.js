...
var  instruction_frame = {
  type: "html-button-response",
  choices: ['Continue'],
  stimulus:  "<p align='left'> Some faces will appear with a red frame around them (shown below). </p>" +
  "<p align='left'> We ask you to only <strong> attend to the faces </strong> and <strong> not to the frames</strong>. </p>" +
   "<img src = img/A75.jpg></img>",
   on_load: frame_intro
};
...
instruction_sequence = instruction_sequence.concat(instruction_seq1, instruction_frame, instruction_seq2, instruction_seq3, instruction_seq4, instruction_seq5, instruction_seq6);
...
function textbox(){
  var textbox = '<style> p { display: block; margin-top: 1em; margin-bottom: 1em; margin-left: 400px; margin-right: 400px;} </style>'
  var imagebox = '<style> #introPics { margin: left;} </style>'
