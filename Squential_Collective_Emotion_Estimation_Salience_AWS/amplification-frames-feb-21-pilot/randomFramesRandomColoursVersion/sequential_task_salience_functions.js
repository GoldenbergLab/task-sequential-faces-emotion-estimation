...
function checkPhone (){
  ...
  function getTimeAndFace (){  //get randomized time of fixation by randomly choosing from 0.4, 0.5 and 0.6s
    ...
    //Face.blocks = shuffle(); // Randomizer | placed here anables randomized trials

    Face.frameRandomizer = getRandomElement(["no_frame","frame"]); // Randomizes if in a frame condition the frame is actually shown
  ...
  function framer (){  //get randomized time of fixation by randomly choosing from 0.4, 0.5 and 0.6s
    if (Face.blocks == "High_Intensity_Frame"){
      if (Face.singleFace > 25){
        frame()
      }
    } else if (Face.blocks == "Low_Intensity_Frame") {
      if (Face.singleFace < 25){
        frame()
      }
    }
    console.log(Face.blocks, Face.singleFace)
  }
  ...
  function shuffle() {
    var order = [];
    order = getRandomElement(["High_Intensity_Frame", "Low_Intensity_Frame","Unchanged_Condition"]);
    return order;
  }

  function frame(){
    Face.frameColour = getRandomElement(["red","blue","green","yellow","black","purple","white"]);
    var frame = '<style> img {border: 5px solid '+ Face.frameColour +'} </style>'

    if (Face.frameRandomizer == "frame") {
      $(".jspsych-content").prepend(frame);
    }
    console.log(Face.frameRandomizer);
  }
...
const DIRECTORY = "amplification-frames-sequence-jschoene-feb-21-pilot"; // CHANGE THE NAME
