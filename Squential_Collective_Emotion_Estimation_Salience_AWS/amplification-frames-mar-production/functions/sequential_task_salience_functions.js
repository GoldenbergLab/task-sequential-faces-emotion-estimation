...
function checkPhone (){
  ...
  function framer (){  //get randomized time of fixation by randomly choosing from 0.4, 0.5 and 0.6s
    if (Face.blocks == "High_Intensity_Frame"){
      if (Face.singleFace >= 30){
        frame()
      }
    } else if (Face.blocks == "Low_Intensity_Frame") {
      if (Face.singleFace <= 20){
        frame()
      }
    }
  ...
  function frame(){
    var frame = '<style> img {border: 5px solid #FF5733} </style>'
...
const DIRECTORY = "amplification-frames-sequence-jschoene-mar-production"; // CHANGE THE NAME
