...
function checkPhone (){
  ...
  function exposureTime (){  //get randomized time of fixation by randomly choosing from 0.4, 0.5 and 0.6s
    if (Face.blocks == "High_Intensity_Prolonged"){
      if (Face.singleFace > 25){
        Face.exposureTime = 2000
      } else {
        Face.exposureTime = 1000
      }
    } else {
      if (Face.singleFace > 25){
        Face.exposureTime = 1000
    ...
    console.log(Face.blocks, Face.singleFace, Face.exposureTime)
    return Face.exposureTime;
  }
  ...
  function shuffle() {
    var order = [];
    order = getRandomElement(["High_Intensity_Prolonged", "Low_Intensity_Prolonged"]);
    return order;
  }
