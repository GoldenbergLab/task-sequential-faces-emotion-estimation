/**
 * jspsych-image-slider-response3
...
jsPsych.plugins['image-slider-response3'] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('image-slider-response3', 'stimulus', 'image');

  plugin.info = {
    name: 'image-slider-response3',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var html =  '<div id="jspsych-image-slider-response3-wrapper" style="margin: 100px 0px;">';   

    //Change: add img stimulus - assign elements of array getScale function in task file to images//
        html += '<div id="jspsych-image-slider-response3-stimulus"><img src="'+trial.stimulus[1]+
    ...
    html += '<div class="jspsych-image-slider-response3-container" style="position:relative;">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-image-slider-response3-response3"></input>';
    html += '<div>'
    ...
    html += '<button id="jspsych-image-slider-response3-button" class="jspsych-btn">'+trial.button_label+'</button>';
    ...
    display_element.querySelector('#jspsych-image-slider-response3-response3').addEventListener('click', function() {
      // measure response time
      var endTime = (new Date()).getTime();
      response1.rt1 = endTime - startTime;
      response1.response1 = display_element.querySelector('#jspsych-image-slider-response3-response3').value;
    ...
    display_element.querySelector('#jspsych-image-slider-response3-button').addEventListener('click', function() {
      // measure response time
      var endTime = (new Date()).getTime();
      response1.rt1 = endTime - startTime;
      response1.response1 = display_element.querySelector('#jspsych-image-slider-response3-response3').value;
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#jspsych-image-slider-response3-stimulus').style.visibility = 'hidden';
