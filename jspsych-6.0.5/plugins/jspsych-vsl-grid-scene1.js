...
jsPsych.plugins['vsl-grid-scene1'] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('vsl-grid-scene1', 'stimuli', 'image');

  plugin.info = {
    name: 'vsl-grid-scene1',
    description: '',
    parameters: {
      stimuli: {
  ...
  plugin.generate_stimulus = function(pattern, image_size) {
    ...
    var html = '<div id="jspsych-vsl-grid-scene1-dummy" css="display: none;">';

    // create table
    html += '<table id="jspsych-vsl-grid-scene1 table" '+
      'style="border-collapse: collapse; margin-left: auto; margin-right: auto;">';

    for (var row = 0; row < nrows; row++) {
      html += '<tr id="jspsych-vsl-grid-scene1-table-row-'+row+'" css="height: '+image_size[1]+'px;">';

      for (var col = 0; col < ncols; col++) {
        html += '<td id="jspsych-vsl-grid-scene1-table-' + row + '-' + col +'" '+
          'style="padding: '+ (image_size[1] / 5) + 'px ' + (image_size[0] / 5) + 'px; border: 0.001px solid #555;">'+
          '<div id="jspsych-vsl-grid-scene1-table-cell-' + row + '-' + col + '" style="width: '+image_size[0]+'px; height: '+image_size[1]+'px;">';
